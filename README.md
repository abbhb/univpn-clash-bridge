...
Each synchronization checks UDP port 53 on each configured DNS IP (two seconds
per server), binding the socket to the VPN or physical interface used by the DNS
policy. ICMP replies are deliberately not accepted: a reachable IP does not prove
that its DNS service is reachable through DIRECT, and Clash can intercept normal
DNS queries. Only DNS responders are used for the domains in
